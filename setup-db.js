...
db.serialize(() => {
  ...
  const users = [
    ...
    ['kasir', 'kasir123', 'Kasir Toko', 'kasir', '081234567891'],
    ['gudang', 'gudang123', 'Petugas Gudang', 'gudang', '081234567892']
  ];
  ...
  insertUser.finalize(() => console.log('✓ Users inserted'));

  // Insert categories
  const insertCategory = db.prepare('INSERT INTO categories (name, description) VALUES (?, ?)');
  categories.forEach((category) => insertCategory.run(category));
  insertCategory.finalize(() => console.log('✓ Categories inserted'));
  ...
  insertSupplier.finalize(() => console.log('✓ Suppliers inserted'));
  ...
  insertProduct.finalize(() => console.log('✓ ' + products.length + ' products inserted'));

  console.log('\n✓ Database Ngebrak Store berhasil dibuat dengan data sehari-hari!\n');
});
